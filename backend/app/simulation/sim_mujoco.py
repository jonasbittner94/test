...
class PackagingSimulation:
    ...
    def _build_scene_xml(self, spawn_states: list[dict]) -> str:
      ...
      <body name="article_{i}" pos="{s["pos"][0]:.6g} {s["pos"][1]:.6g} {s["pos"][2]:.6g}"
        ...
        shaker = f'''
        <body name="shaker" pos="0 0 0">
        <joint name="shaker_x" type="slide" axis="1 0 0" damping="0"/>
        <joint name="shaker_y" type="slide" axis="0 1 0" damping="0"/>
        ...
        <!-- zusätzliche äußere Sicherheitswände -->
        <geom type="box" size="{safety_half_t:.6g} {half_y:.6g} {wall_height / 2:.6g}"
              pos="{half_x + cfg.wall_thickness + safety_wall_gap + safety_half_t:.6g} 0 {wall_center_z:.6g}"
              {friction} rgba="0 0 0 0"/>
        <geom type="box" size="{safety_half_t:.6g} {half_y:.6g} {wall_height / 2:.6g}"
              pos="{-half_x - cfg.wall_thickness - safety_wall_gap - safety_half_t:.6g} 0 {wall_center_z:.6g}"
              {friction} rgba="0 0 0 0"/>
        <geom type="box" size="{half_x:.6g} {safety_half_t:.6g} {wall_height / 2:.6g}"
              pos="0 {half_y + cfg.wall_thickness + safety_wall_gap + safety_half_t:.6g} {wall_center_z:.6g}"
              {friction} rgba="0 0 0 0"/>
        <geom type="box" size="{half_x:.6g} {safety_half_t:.6g} {wall_height / 2:.6g}"
              pos="0 {-half_y - cfg.wall_thickness - safety_wall_gap - safety_half_t:.6g} {wall_center_z:.6g}"
              {friction} rgba="0 0 0 0"/>
        ...
        scraper_half_x = cfg.box_x * 0.6
        scraper_half_y = cfg.box_y * 0.6
        scraper_half_z = 0.0015
        scraper_start_x = -(cfg.box_x + 0.05)
        scraper_start_z = wall_height + 0.05

        scraper = f'''
        <body name="scraper" pos="{scraper_start_x:.6g} 0 {scraper_start_z:.6g}">
          <joint name="scraper_x" type="slide" axis="1 0 0" damping="0"/>
          <joint name="scraper_z" type="slide" axis="0 0 1" damping="0"/>
          <geom type="box"
                size="{scraper_half_x:.6g} {scraper_half_y:.6g} {scraper_half_z:.6g}"
                pos="0 0 0"
                friction="{ARTICLE_FRICTION} {SPINNING_FRICTION} {ROLLING_FRICTION}"
                condim="6"
                rgba="1.0 0.3 0.3 0.35"/>
        </body>'''
